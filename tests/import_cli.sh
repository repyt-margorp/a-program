...
expect 3 --steps 0 --imports "$directory/provider.p" --save "$directory/invalid-provider.a" "$directory/client.p"
expect 1 --load "$directory/invalid-provider.a"
...
printf '%s\n' 'Bool:=@{false:*; true:*;}; id:=\x:Bool=>x; value:=id Bool.true; delayed:=&(id Bool.true);' > "$directory/computed.p"
printf '%s\n' 'import Bool; import id; import value; main:=id value; main::Bool;' > "$directory/computed-client.p"
"$binary" --imports "$directory/computed.p" --save "$directory/computed.a" "$directory/computed-client.p" > "$directory/status"
"$binary" --load "$directory/computed.a" > "$directory/status"
expect 3 --steps 0 --imports "$directory/computed.p" --save "$directory/computed-pending.a" "$directory/computed-client.p"
"$binary" --load "$directory/computed-pending.a" > "$directory/status"
printf '%s\n' 'import id; import delayed; main:=id delayed;' > "$directory/quoted-client.p"
expect 1 --imports "$directory/computed.p" "$directory/quoted-client.p"
expect 3 --steps 0 --imports "$directory/computed.p" --save "$directory/quoted.a" "$directory/quoted-client.p"
expect 1 --load "$directory/quoted.a"
echo 'import cli: computation definitions retain their source polarity; explicit quotations remain values'
