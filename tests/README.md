# Pointer compiler verification

Run `make -f src/Makefile check` from the repository root.
The compiler does not invoke the old compiler or link its solver/runtime.
Compatibility tests read the preserved `.p` corpus under
`archive/legacy/src/prototype/tests/fixtures/`; no archived C implementation is built.
...
`compatibility.tsv` is generated by `sh tests/inventory.sh`.
...
not scan build output or infer expected behavior from a filename. The name column
retains historical paths; the source column maps them to their current archive
...
`make -f src/Makefile parse-files` builds `.build/parse_files`.
