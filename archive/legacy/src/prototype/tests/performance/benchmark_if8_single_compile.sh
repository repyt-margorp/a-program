...
while [ "$run" -le 3 ]; do
	...
	counter_lines=$(grep '^A_PROGRAM_\(PERFORMANCE\|SOLVER\|SOLVER_KIND\|CONTEXT_RESOLUTION\|CONSTRUCTOR_SPECIALIZATION\|COMPILE_PHASE\|PROOF_MATERIALIZATION\|ACCEPTED_REPLAY\)_COUNTERS 1 ' \
		"$metrics")
	printf '%s\n' "$counter_lines"
