...
./read_file.out --write-artifact "$tmp_dir/map.apo" \
	src/prototype/tests/fixtures/typing/explicit_index_family_map_check.p \
	>"$tmp_dir/map.out"
grep -Eq '^term map .* evidence 1 [0-9]+ 0$' "$tmp_dir/map.apo"
grep -qx 'verification_obligations 0' "$tmp_dir/map.apo"

conditional_source=src/prototype/tests/fixtures/cbpv/runtime_strict_effects_check.p
conditional_export=matchScrutineeEffect
...
grep -Eq '^verification 0 1 1 ' "$tmp_dir/conditional.apo"
...
printf '%s\n' 'alias := matchScrutineeEffect;' >"$tmp_dir/conditional-consumer.p"
...
# occurrence, but cannot replace another occurrence's exact residual.
...
fold_obligation=$(awk \
	'$1 == "term" && $2 == "main" && $(NF - 4) == "evidence" { print $NF; exit }' \
	"$tmp_dir/mixed.apo")
...
expect_read_failure "$tmp_dir/unrelated-condition.apo" \
	'unrelated computation-fold obligation authorized a conditional export'
...
awk '
	$1 == "term" && $2 == "matchScrutineeEffect" { $4 = 0 }
	{ print }
...
expect_read_failure "$tmp_dir/wrong-effect-kind.apo" \
	'computation condition accepted a nonzero reserved effect constraint kind'
...
expect_read_failure "$tmp_dir/wrong-effect-profile.apo" \
	'computation condition accepted a noncanonical normalization profile'
