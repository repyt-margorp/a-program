...
For two admitted endpoint classifiers:

```text
Gamma |-v A0 type       Gamma |-v A1 type
Gamma |-v a0 : A0       Gamma |-v a1 : A1
------------------------------------------------
Gamma |-v ObsV(A0, A1, a0, a1) type
```
...
The object TermDB forms are exactly:

```text
OBSERVATION_TYPE(A0, A1, a0, a1)
OBSERVATION_WITNESS(a0, a1)
```

All operands are ordinary TermDB IDs. Context, Substitution, Operation, Claim,
Derivation, action, and work-item IDs are forbidden from these keys.
Formation is type-directed by exact accepted endpoint Claims and the admitted
type-former descriptor. The term is not a Boolean comparison and does not
extend kernel conversion.

`ap` is the compiler action that translates one exact accepted source Claim
and recursively constructs `OBSERVATION_WITNESS` terms. The witness term does
not store its request ID; its exact `HAS_TYPE` Derivation records the source
and endpoint Claims plus the rule-specific recursive witness premises. The
separate compiler action certificate points to that accepted witness Claim;
Judgement proofs do not embed compiler action IDs. Constructing the TermDB node
alone is never evidence.

Homogeneous equality and reflexivity are derived:

```text
Eq(A, a, b) = ObsV(A, A, a, b)
refl(a)     = OBSERVATION_WITNESS(a,a), generated by diagonal ap
```

No generic `JUDGEMENT_EQ` and no separate first-fragment `REFL` node is added. Applying
`OBSERVATION_TYPE` to endpoints that are themselves observation types uses the
same type-directed rule; witness equality is therefore not collapsed to Unit,
proof irrelevance, or raw TermDB identity.
...
C01 |- AR = ObsV(A[pi0], A[pi1], x0, x1) type
```
...
For every admitted type and term, Type action first constructs the endpoint
Context and only then forms its relation:

```text
Gamma |-v A type
  gives C01 = Gamma^R.x0:A[pi0].x1:A[pi1] and C01 |-v AR type

Gamma |-v a : A
  gives Gamma^R |-v a^R : AR[a[pi0], a[pi1]]
```

Consequently, a Type-action result owns `C01`, both endpoint bindings, `AR`,
and the exact formation authority. A bare relation Term without this Context
is not a valid action result.
...
(a[sigma])^R = a^R[sigma^R]
ObsV(A0,A1,a,b)[sigma]
  = ObsV(A0[sigma], A1[sigma], a[sigma], b[sigma])
```
