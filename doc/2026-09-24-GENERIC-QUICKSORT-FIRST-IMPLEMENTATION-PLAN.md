...
### Q4: Universal QuickSort Content Preservation

Baseline: `c21d35e`. This closes the missing general-content proof gate in
#34, not the entire issue or the A3-A5/R2-R5 authority refactor. All new proof
code is ordinary A Program under `tests/fixtures/generic_sorted/`; compiler,
kernel, artifact formats and the frozen provider remain unchanged.

`content-proof.p` defines the following theorem:

```text
quick_content :
  (A : @) -> (le : A -> A -> Bool) ->
  (input : List A) -> (output : List A) ->
  @quickSort A (&le) input output -> permutation A input output
```

`permutation` is an ordinary IADT generated by empty lists, preserving a
shared head, adjacent swaps and composition. These constructors preserve
elements and multiplicities; a length proof or preservation of element
predicates alone does not. No quotient or identification of evidence trees is
introduced. The theorem is conditional on the supplied function-graph witness;
it does not assert that an arbitrary effectful comparator terminates.

The proof relates `SizedList` data to ordinary lists with `sized_contents`,
then inducts over the existing measurement, partition, append and recursive
QuickSort graphs. Its partition specification mentions actual data, not LT
bound-proof history. The derived-provider patch adds only the two extra
helper-field bindings to partition graph clauses; the proof body is shared.

- [x] Check the universally quantified theorem with both providers, without
  an assumed partition theorem or comparator correctness certificate.
- [x] Eliminate the resulting witnesses to recover outputs for empty,
  singleton, reversed and duplicate inputs, and the original duplicate input.
- [x] Reject wrong-output, changed-element and changed-multiplicity claims.
  The multiplicity negative keeps the same length and set of Bool values.
...
`permutation values values` instead of `permutation values nil`. An explicit
source motive selects the latter; `::` remains a post-check. Recomputing
flattened lists inside indices also encountered neutral sequencing expressions
that the current conversion rules do not identify. The relational proof avoids
requiring those additional equations. Finally, passing the already constructed
partition theorem into a refined branch reuses its exact nominal graph source,
instead of re-quoting a different source under newly refined binders. None of
these experiments established a kernel soundness bug or justifies adding
commuting conversions to DefEq.

Focused O2 matrix: `/tmp/a-program-content-derived-opt.log`, exit 0. Combined
Sorted/content fixture checking takes 820,427 steps with the frozen provider
and 824,974 with the derived provider. These enlarged fixtures are not a
compiler-speed comparison against the earlier Sorted-only experiment.
Fresh strict `-O2` full `check-acceptance` passes, including 63/63 compatibility
cases (`/tmp/a-program-content-acceptance-opt.log`). Focused Debug and
ASan/UBSan runs pass both providers, empty/reversed/duplicate witness consumers,
300,000-step retained resumption and all three new source negatives
(`/tmp/a-program-content-focus-{debug,sanitize}.log`). Leak detection and
halt-on-error were enabled. The latter builds reuse the verified `c21d35e`
compiler: C and C-test sources were compared byte-for-byte before reuse.
Partition-change sensitivity and the final LT representation decision remain
open. Implementation C/H delta is zero; the added proof/test code is not a
source-reduction result.

| File under `src/prototype/pointer/tests/` | Added | Deleted | Net |
| --- | ---: | ---: | ---: |
| `fixtures/generic_sorted/content-proof.p` | 206 | 0 | +206 |
| `fixtures/generic_sorted/boolean-content-consumer.p` | 20 | 0 | +20 |
| `fixtures/generic_sorted/boolean-wrong-content-output.p` | 3 | 0 | +3 |
| `fixtures/generic_sorted/boolean-wrong-content-element.p` | 2 | 0 | +2 |
| `fixtures/generic_sorted/boolean-wrong-content-multiplicity.p` | 5 | 0 | +5 |
| `fixtures/generic_sorted/derived-lt.patch` | 14 | 0 | +14 |
| `derived_lt.sh` | 9 | 4 | +5 |
