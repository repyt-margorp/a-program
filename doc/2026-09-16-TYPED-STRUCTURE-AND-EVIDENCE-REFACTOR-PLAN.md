...
| `function_graph.c:computation_origin` | Deleted in favor of shared checked construction access. A4 advances structural input/origin queries incrementally and retains only the borrowed in-flight query during suspension; no per-poll origin/view reconstruction. Helper-call spine analysis and checked specialization are still distinct operations, not another construction authority |
