...
grep -q '^A_PROGRAM_ARTIFACT 57$' "$TMP_DIR/identity.apo"
...
sed '1s/57$/56/' "$TMP_DIR/identity.apo" >"$TMP_DIR/identity-v56.apo"
if ./read_file.out --read-graph "$TMP_DIR/identity-v56.apo" >"$TMP_DIR/identity-v56.out" 2>"$TMP_DIR/identity-v56.err"; then
	echo "obsolete artifact unexpectedly passed after v57 format bump" >&2
	exit 1
fi
