...
grep -q '^A_PROGRAM_ARTIFACT 56$' "$TMP_DIR/identity.apo"
...
sed '1s/56$/55/' "$TMP_DIR/identity.apo" >"$TMP_DIR/identity-v55.apo"
if ./read_file.out --read-graph "$TMP_DIR/identity-v55.apo" >"$TMP_DIR/identity-v55.out" 2>"$TMP_DIR/identity-v55.err"; then
	echo "obsolete artifact unexpectedly passed after v56 format bump" >&2
	exit 1
fi
