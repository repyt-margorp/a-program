...
grep -q '^A_PROGRAM_ARTIFACT 54$' "$tmp_dir/selected-cutoff.apo"
...
grep -q '^A_PROGRAM_ARTIFACT 54$' "$tmp_dir/match-exit.apo"
...
reject_case lambda-exit-under-handler \
	'main := \x : #.Text => handle ({ !x; }) with (#.print) y k => k y; return z => z;'
