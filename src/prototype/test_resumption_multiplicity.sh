...
do
	./read_file.out "src/prototype/$source.p" >"$tmp_dir/$source.out"
	grep -q '^term main := COMPUTATION_FOLD(' "$tmp_dir/$source.out"
done

if ./read_file.out src/prototype/resumption_one_shot_invalid.p \
...
if ./read_file.out src/prototype/resumption_abortive_invalid.p \
...
for source in \
	...
	resumption_abortive_check
do
	printf ':q\n' | ./a.out "src/prototype/$source.p" >"$tmp_dir/$source.eval"
done
...
./read_file.out --write-artifact "$tmp_dir/multi.apo" \
	src/prototype/resumption_multi_shot_check.p >"$tmp_dir/multi-write.out"
