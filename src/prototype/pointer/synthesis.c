...
static const struct pg_evidence *match_branch_result(struct pg_synthesis *synthesis,
	const struct match_state *state, const struct match_branch *branch)
{
...
static void match_dependent_branch(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	if (!branch->converted) {
		...
		struct pg_synthesis_job *body = pg_synthesis_abstract(synthesis, fields, fields, branch->body);
		const struct pg_evidence *result_type = match_branch_result(synthesis, state, branch);
...
static void match_validate_branch(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	if (!branch->converted) {
		...
		const struct pg_evidence *map = allocation
			? pg_prove_induction_scope_at(synthesis->typing, synthesis->classifiers,
			...
			: state->induction ? pg_prove_induction_scope(synthesis->typing, synthesis->classifiers,
		...
		const struct pg_evidence *expected = pg_prove_match_branch_type(synthesis->typing, synthesis->classifiers,
...
static int match_generalize(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	work->back = pg_prove_inductive_motive_substitution(typing, synthesis->classifiers,
	...
	if (job->match && job->match->specialization) {
		...
		if (!job->value_job) {
			...
			for (size_t i = end - state->generalized_count; i < end; ++i)
				result = pg_synthesis_application(synthesis, source_context(job->inner), result,
...
static void constructor_transport_step(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	if (field) {
		...
		field_type = pg_prove_classifier(typing, synthesis->classifiers, context, value);
	...
	const struct pg_evidence *type = pg_prove_classifier(typing, synthesis->classifiers, context, job->left->result);
	struct pg_inductive_instance instance;
	...
	const struct pg_evidence *source = pg_prove_classifier(typing, synthesis->classifiers, context, value);
	...
	const struct pg_evidence *family = pg_prove_type_case(typing, synthesis->classifiers, instance.formation,
...
static void index_argument_step(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	if (!job->left) goto error;
	if (job->left->status == PG_SYNTHESIS_PENDING) { depend(synthesis, job, job->left); return; }
	if (job->left->status != PG_SYNTHESIS_REJECTED) { forward_proof(synthesis, job, job->left); return; }
	...
	if (!state->path) {
		...
		const struct pg_evidence *formation = pg_identity_formation(typing, synthesis->classifiers,
	...
	struct pg_inductive_instance instance;
	const struct pg_evidence *type = pg_prove_classifier(typing, synthesis->classifiers, context, state->normal[0]->result);
...
static int source_has_identity(struct pg_synthesis *synthesis, const struct pg_source_scope *scope)
{
	for (; scope; scope = scope->parent) {
		...
		if (pg_identity_formation(synthesis->typing, synthesis->classifiers, pg_evidence_premise(context, 1))) return 1;
	}
	return 0;
}
