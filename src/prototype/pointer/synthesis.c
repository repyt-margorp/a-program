...
static const struct pg_evidence *continuation_function(struct pg_synthesis *synthesis,
	const struct continuation_frame *frame, const struct pg_evidence *body)
{
	...
	return pg_prove_lambda(synthesis->typing, pi, body);
}
...
{
	if (!job->continuation) {
		job->continuation = continuation_function(synthesis, frame, body);
...
static void handler_return_step(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	job->result = continuation_function(synthesis, job->application_frame, body);
	finish(synthesis, job, job->result ? PG_SYNTHESIS_DONE : PG_SYNTHESIS_UNSUPPORTED);
	return;
rejected:
	finish(synthesis, job, PG_SYNTHESIS_REJECTED);
...
static void handler_clause_step(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	struct pg_synthesis_job *inputs[] = {job->left, carrier};
	for (size_t i = 0; i < 2; ++i) {
	...
	struct pg_synthesis_job *canonical = pg_synthesis_handler_clause(synthesis, job->scope,
		pg_synthesis_evidence(synthesis, carrier->result), clause);
	if (forward_proof(synthesis, job, canonical)) return;
	if (!job->inner) {
		...
		const struct pg_evidence *context = pg_prove_handler_context(synthesis->typing, synthesis->classifiers,
			operation, source_context(job->scope), carrier->result, payload, resume);
		if (!context) goto rejected;
		const struct pg_source_scope *scope = pg_synthesis_bind(synthesis, job->scope,
			clause->items[0].name, payload, pg_evidence_premise(context, 0));
		job->inner = pg_synthesis_bind(synthesis, scope, clause->items[1].name, resume, context);
		if (!job->inner) { finish(synthesis, job, PG_SYNTHESIS_ERROR); return; }
		job->right = pg_synthesis_request(synthesis, job->inner, clause->right);
		depend(synthesis, job, job->right);
		return;
	...
	if (job->right->status != PG_SYNTHESIS_DONE) { finish(synthesis, job, job->right->status); return; }
	job->result = pg_prove_abstract(synthesis->typing, synthesis->classifiers,
		source_context(job->scope), source_context(job->inner), computation(synthesis, job->right->result));
	finish(synthesis, job, job->result ? PG_SYNTHESIS_DONE : PG_SYNTHESIS_REJECTED);
	return;
rejected:
	finish(synthesis, job, PG_SYNTHESIS_REJECTED);
...
static void classifier_structure_step(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	if (producer->role == BODY_JOB) {
		struct pg_synthesis_job *body = (void *)producer->inputs[0];
		struct pg_synthesis_job *rule = prepared_source_rule(body);
		if (!rule) rule = body;
		if (rule->role == DERIVATION_JOB) {
			const struct pg_derivation_input *body_input = rule->inputs[0];
			int returns_value = 0, known = 1;
			switch (body_input->rule) {
			case PG_VARIABLE: case PG_THUNK_INTRO: case PG_VALUE_FROM_TYPE: case PG_UNIVERSE_FORM:
				returns_value = 1; break;
			case PG_LAMBDA_INTRO: case PG_APP_ELIM: case PG_FORCE_ELIM: case PG_RETURN_INTRO: break;
	...
	if (!job->left && input) {
		...
		case PG_FORCE_ELIM: case PG_THUNK_INTRO: case PG_APP_ELIM:
...
static void type_structure_step(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	if (producer->role == DOMAIN_JOB) {
		...
		if (rule && rule->role == DERIVATION_JOB) {
			...
			if (domain->rule == PG_UNIVERSE_FORM || domain->rule == PG_VARIABLE) {
				if (!job->left) job->left = domain->rule == PG_VARIABLE
					? pg_synthesis_term_structure(synthesis, rule) : pg_synthesis_type_structure(synthesis, rule);
				forward_structure(synthesis, job);
				return;
			}
		}
...
static int prepare_expression(struct pg_synthesis *synthesis, struct pg_synthesis_job *job)
{
	...
	if (syntax->kind == PG_SYNTAX_LAMBDA) {
		struct pg_synthesis_job *body = request_job(synthesis, BODY_JOB, job->right, NULL);
		if (!body) goto error;
		struct pg_synthesis_job *codomain = request_job(synthesis, CLASSIFIER_FORMATION_JOB, job->left, body);
		if (!codomain) goto error;
		struct pg_derivation_input pi_input = {.rule = PG_PI_FORM, .count = 3};
		struct pg_synthesis_job *premises[] = {job->left->right, job->left, codomain};
		...
		job->value_job = pg_synthesis_rule(synthesis, &lambda_input, lambda_premises, NULL, NULL);
