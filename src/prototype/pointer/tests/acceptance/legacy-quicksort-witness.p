...
graph := @quickSortAcc;
main := *quickSortAcc Nat &lessOrEqual two (natAccessible two) input @output => output;
expected := (List Nat).cons one ((List Nat).cons two (List Nat).nil);
emptyMain := *quickSortAcc Nat &lessOrEqual Nat.zero (natAccessible Nat.zero) (SizedList Nat).nil @output => output;
