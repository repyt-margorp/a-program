...
static int term_reference(FILE *file, const struct pg_term *term,
	const struct pg_term **terms, size_t *count)
{
	if (!term) return pg_wire_write_u64(file, 0);
	terms[(*count)++] = term;
	return pg_wire_write_u64(file, *count);
}
	...
	const char *(*name)(void *, const struct pg_object *), void *owner)
{
	...
	int status = -1;
	if (pg_dag_init(&dag, premise, NULL) || pg_graph_init(&arena)) goto done;
	for (size_t i = 0; i < count; ++i) if (pg_dag_add(&dag, roots[i])) goto done;
	if (dag.count > SIZE_MAX / sizeof(void *) / 3) goto done;
	const struct pg_term **terms = pg_alloc(&arena, 3 * dag.count * sizeof(*terms));
	if (!terms) goto done;
	size_t term_count = 0;
	...
	for (const struct pg_dag_node *node = dag.first; node; node = node->next) {
		...
		if (term_reference(file, binder, terms, &term_count)
			|| term_reference(file, source, terms, &term_count)
			|| term_reference(file, target, terms, &term_count)) goto done;
	...
	status = pg_graph_write(file, term_count, terms, name, owner);
done:
