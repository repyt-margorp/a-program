	...
	const uint64_t **metadata, size_t *root_count, const struct pg_term *const **roots)
{
	...
	size_t n = dag.count;
	if (count > SIZE_MAX / sizeof(uint64_t) - 2 || n > SIZE_MAX / sizeof(uint64_t) - 2 - count) goto done;
	if (term_count > SIZE_MAX / sizeof(void *) || n > (SIZE_MAX / sizeof(void *) - term_count) / 2) goto done;
	uint64_t *ids = pg_alloc(storage, (2 + n + count) * sizeof(*ids));
	...
	*metadata_count = 2 + n + count; *metadata = ids;
	...
	size_t *term_count, const struct pg_term *const **terms)
{
	...
	if (n > metadata_count - 2 || nc != metadata_count - 2 - n || n > root_count / 2) return -1;
	if (n > SIZE_MAX / sizeof(void *) || nc > SIZE_MAX / sizeof(void *)) return -1;
	for (size_t i = 0; i < n; ++i) if (metadata[2 + i] > i) return -1;
	for (size_t i = 0; i < nc; ++i) if (metadata[2 + n + i] > n) return -1;
	...
	for (size_t i = 0; i < n; ++i) {
		...
		uint64_t prefix = metadata[2 + i];
		all[i] = pg_context_bind(typing, prefix ? all[prefix - 1] : NULL, binder->as.reference, roots[2 * i + 1]);
		if (!all[i]) return -1;
	}
	for (size_t i = 0; i < nc; ++i) {
		uint64_t id = metadata[2 + n + i];
