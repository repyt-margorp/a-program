	...
	const uint64_t **metadata, size_t *root_count, const struct pg_term *const **roots)
{
	...
	size_t n = dag.count;
	if (count > SIZE_MAX / sizeof(uint64_t) - 2 || n > (SIZE_MAX / sizeof(uint64_t) - 2 - count) / 2) goto done;
	if (term_count > SIZE_MAX / sizeof(void *) || n > (SIZE_MAX / sizeof(void *) - term_count) / 2) goto done;
	uint64_t *ids = pg_alloc(storage, (2 + 2 * n + count) * sizeof(*ids));
	...
	for (const struct pg_dag_node *r = dag.first; r; r = r->next) {
		...
		ids[2 * r->id] = prefix ? prefix->id : 0;
		ids[2 * r->id + 1] = context->judgement;
	...
	for (size_t i = 0; i < count; ++i) {
		...
		ids[2 + 2 * n + i] = r ? r->id : 0;
	}
	for (size_t i = 0; i < term_count; ++i) {
		if (!terms[i]) goto done;
		output[2 * n + i] = terms[i];
	}
	*metadata_count = 2 + 2 * n + count; *metadata = ids;
	...
	size_t *term_count, const struct pg_term *const **terms)
{
	...
	if (n > (metadata_count - 2) / 2 || nc != metadata_count - 2 - 2 * n || n > root_count / 2) return -1;
	if (n > SIZE_MAX / sizeof(void *) || nc > SIZE_MAX / sizeof(void *)) return -1;
	for (size_t i = 0; i < n; ++i) {
		if (metadata[2 + 2 * i] > i) return -1;
		uint64_t kind = metadata[3 + 2 * i];
		if (kind != PG_JUDGEMENT_VALUE && kind != PG_JUDGEMENT_TYPE_FAMILY) return -1;
	}
	for (size_t i = 0; i < nc; ++i) if (metadata[2 + 2 * n + i] > n) return -1;
	...
	for (size_t i = 0; i < n; ++i) {
		...
		uint64_t prefix = metadata[2 + 2 * i];
		all[i] = pg_context_bind(typing, prefix ? all[prefix - 1] : NULL, binder->as.reference,
			roots[2 * i + 1], metadata[3 + 2 * i]);
		if (!all[i]) return -1;
	}
	for (size_t i = 0; i < nc; ++i) {
		uint64_t id = metadata[2 + 2 * n + i];
