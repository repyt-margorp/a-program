...
const struct pg_context_map *pg_context_map_extend(struct pg_typing *typing,
	...
	const struct pg_occurrence *const *suffix)
{
	if (!map || (count && !suffix)) return NULL;
	size_t length;
	if (pg_context_extension_size(source, map->source, &length) || length != count) return NULL;
	if (pg_context_extension_size(destination, map->destination, &length)) return NULL;
	if (!count && destination == map->destination) return map;
	if (count > SIZE_MAX / sizeof(const struct pg_occurrence *) - map->count) return NULL;
	size_t total = map->count + count;
	const struct pg_occurrence **images = malloc(total * sizeof(*images));
	if (total && !images) return NULL;
	for (size_t i = 0; i < map->count; ++i)
		images[i] = destination == map->destination ? map->images[i]
			: pg_occurrence_weaken(typing, destination, map->images[i]);
	for (size_t i = 0; i < count; ++i) images[map->count + i] = suffix[i];
	const struct pg_context_map *result = pg_context_map(typing, source, destination, total, images);
	free(images);
	return result;
}
...
static enum pg_substitution_status context_lift_step(struct pg_context_lift *work)
{
	...
	work->result = pg_context_map_extend(typing, work->map, work->extension, destination, 1, &image);
	...
	const struct pg_occurrence *body, const struct pg_occurrence *argument)
{
	...
	return pg_occurrence_action_request(typing,
		pg_context_map_extend(typing, prefix, body->context, argument->context, 1, &argument), body);
}
...
struct pg_occurrence_input {
	...
	struct pg_typing *typing;
	const struct pg_occurrence *source, *current, *result, *child;
	...
	const struct pg_occurrence *child)
{
	...
	for (struct pg_index_entry *p = pg_index_candidates(&typing->occurrence_inputs, hash); p; p = p->next) {
		...
		if (p->hash == hash && work->source == source && work->index == index && work->outer == map && work->child == child) return work;
	}
	struct pg_occurrence_input *work = pg_alloc(typing->graph, sizeof(*work));
	if (!work) return NULL;
	work->typing = typing;
	work->source = source;
	work->current = child ? NULL : source;
...
{
	return index == SIZE_MAX ? NULL : input_request(typing, source, index, NULL, NULL);
}
	...
	const struct pg_occurrence *source)
{
	...
	return input_request(typing, source, SIZE_MAX, NULL, NULL);
}
...
static enum pg_occurrence_input_status occurrence_input_step(struct pg_occurrence_input *work)
{
	...
	if (work->current) {
		...
		if (current->selection && work->index != SIZE_MAX) {
			if (!work->selected) work->selected = input_request(typing, current->origin, current->selection - 1, NULL, NULL);
