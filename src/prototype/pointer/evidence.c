...
static const struct pg_evidence *formed_classifier(struct pg_typing *typing,
	struct pg_classifiers *classifiers, const struct pg_evidence *term);
	...
	const struct pg_evidence *context, const struct pg_evidence *image)
{
	struct image_frame {
		...
		const struct pg_evidence *source, *substitution, *constructor;
	...
	while (image) {
		...
		*next = (struct image_frame){.expected = expected, .source = pg_prove_structural_subject(typing, subject), .parent = frame};
		...
		if (!subject->origin && head->kind == PG_REFERENCE && pg_data_constructor_view(head->as.reference, &layout, &position, &arity)) {
			...
			next->constructor = constructor_origin(typing, next->source);
			if (!next->constructor) goto done;
			next->parameters = next->constructor->premises[2]->premise_count - 2;
		...
		for (size_t i = 0; i < next->count; ++i) {
			...
			if (next->constructor && i < next->parameters) input = next->constructor->premises[2]->premises[i + 2];
...
children:
	...
	if (frame->constructor) {
		const struct pg_evidence *map = pg_prove_substitution(typing, frame->constructor->premises[2]->premises[0],
			context, frame->parameters, frame->outputs);
		result = pg_prove_constructor(typing, frame->constructor->premises[1],
			pg_evidence_constructor(frame->constructor), map, frame->count - frame->parameters,
	...
	const struct pg_evidence *value)
{
	...
	/* Reuse an already checked introduction of this exact typed construction.
	 * Other receipts remain accepted alternatives, not a source of operands. */
	for (const struct pg_evidence *proof = pg_evidence_for_subject(typing, pg_evidence_subject(value), NULL);
		proof; proof = pg_evidence_for_subject(typing, pg_evidence_subject(value), proof))
		if (proof->rule == PG_CONSTRUCTOR_INTRO) return proof;
	struct pg_graph temporary = {0};
	struct construction_map *frames = NULL;
	const struct pg_evidence *result = NULL;
	...
	const struct pg_object *scrutinee)
{
	...
	if (pattern->premises[1] != value->premises[1]) return NULL;
	const struct pg_evidence *left = pattern->premises[3], *right = value->premises[3];
	if (left->premise_count != right->premise_count) return NULL;
	struct pg_graph temporary = {0};
	...
	const struct pg_evidence *value)
{
	...
	if (pg_evidence_judgement(value) != PG_JUDGEMENT_VALUE) return NULL;
	if (!classifiers || classifiers->graph != typing->graph) return NULL;
	const struct pg_evidence *type = pg_prove_computation_type(typing, classifiers, totality,
		pg_effect_row(typing->graph, 0, NULL), formed_classifier(typing, classifiers, value));
	if (!type) return NULL;
	...
	struct pg_classifiers *classifiers, const struct pg_evidence *computation)
{
	...
	return unary_term(typing, computation, &pg_thunk_operation,
		pg_prove_thunk_type(typing, classifiers, formed_classifier(typing, classifiers, computation)), PG_THUNK_INTRO, PG_JUDGEMENT_VALUE);
}
...
const struct pg_evidence *pg_prove_force(struct pg_typing *typing, const struct pg_evidence *value)
{
	...
	return unary_term(typing, value, &pg_force_operation,
		pg_prove_thunk_content(typing, formed_classifier(typing, NULL, value)), PG_FORCE_ELIM, PG_JUDGEMENT_COMPUTATION);
}
	...
	const struct pg_evidence *computation)
{
	...
	if (!core) return NULL;
	const struct pg_evidence *formation = pg_prove_return_content(typing, formed_classifier(typing, NULL, computation));
	if (!formation) return NULL;
	...
	const struct pg_evidence *function, const struct pg_evidence *argument)
{
	...
	const struct pg_evidence *type = pg_prove_pi_codomain(typing, formed_classifier(typing, NULL, function), argument);
	...
	size_t count, const struct pg_evidence *const *images)
{
	...
	for (size_t i = 0; i < retained; ++i) premises[i + 2] = prefix->premises[i + 2];
	for (size_t i = 0; i < count; ++i) {
	...
	const struct pg_evidence *destination, const struct pg_evidence *image)
{
	...
	struct pg_graph temporary = {0};
	const struct pg_evidence **premises = pg_alloc(&temporary, (count + 3) * sizeof(*premises));
	const struct pg_occurrence **typed_images = pg_alloc(&temporary, (count + 1) * sizeof(*typed_images));
	const struct pg_evidence *result = NULL;
	if (!premises || !typed_images) goto done;
	...
	for (size_t i = 0; i < count; ++i) {
		...
		typed_images[i] = pg_evidence_subject(premises[i + 2]);
	}
	premises[count + 2] = image;
	typed_images[count] = pg_evidence_subject(image);
	uint64_t hash;
	result = find_record(typing, PG_CONTEXT_SUBSTITUTION,
		pg_evidence_context(destination), NULL, count + 3, premises, NULL, &hash);
	if (result) goto done;
	const struct pg_context_map *map = pg_context_map(typing, pg_evidence_context(source_extension),
		pg_evidence_context(destination), count + 1, typed_images);
	if (!map) goto done;
	const struct pg_term *expected = pg_substitution_compute(&typing->substitutions,
		pg_evidence_context(source_extension)->declared_type, count, pg_context_map_bindings(map));
	if (!expected) goto done;
	if (pg_alpha_equal(expected, pg_evidence_subject(image)->classifier) != 1) goto done;
	result = accept_record(typing, PG_CONTEXT_SUBSTITUTION,
	...
	const struct pg_evidence *payload, const struct pg_evidence *continuation)
{
	...
	const struct pg_evidence *type = pg_prove_pi_constant_codomain(typing, formed_classifier(typing, classifiers, continuation));
	type = pg_prove_computation_type(typing, classifiers, totality, row, pg_prove_return_content(typing, type));
	...
	const struct pg_evidence *computation, const struct pg_evidence *continuation)
{
	...
	if (pg_computation_type_view(codomain, &next_totality, &following, &result_type)) {
		...
		type = pg_prove_computation_type(typing, classifiers, totality,
...
int pg_classifier_recovery_init(struct pg_classifier_recovery *work,
	...
	const struct pg_evidence *context, const struct pg_evidence *term)
{
	if (!classifiers || !typing || classifiers->graph != typing->graph) {
		*work = (struct pg_classifier_recovery){.status = -1};
		return -1;
	}
	return classifier_initialize(work, typing, classifiers, context, term);
}
...
/* Computation classifiers have retained formations. Universe-valued terms
 * alone need the Universe owner to construct their next formation lazily. */
static const struct pg_evidence *formed_classifier(struct pg_typing *typing,
	struct pg_classifiers *classifiers, const struct pg_evidence *term)
{
	if (!pg_evidence_owned_by(term, typing)) return NULL;
	...
	classifier_initialize(&work, typing, classifiers, context, term);
