...
grep -q 'has-type RETURN(INT_LITERAL(1)) COMPUTATION_TYPE(EFFECT_ROW_EMPTY, PRIMITIVE(Int64)) \[return-intro proof#' "$tmp_dir/output"
grep -q 'has-type THUNK(RETURN(INT_LITERAL(1))) Thunk(COMPUTATION_TYPE(EFFECT_ROW_EMPTY, PRIMITIVE(Int64))) \[thunk-intro proof#' "$tmp_dir/output"
grep -q 'has-type FORCE(THUNK(RETURN(INT_LITERAL(1)))) COMPUTATION_TYPE(EFFECT_ROW_EMPTY, PRIMITIVE(Int64)) \[force-elim proof#' "$tmp_dir/output"
