...
static struct prototype_substitution_certificate substitution_certificates[128];
static struct prototype_context_formation_certificate context_certificates[128];
...
int main(void) {
	...
	prototype_substitution_certificate_db_init(
		&substitution_certificate_db, substitution_certificates, 128
	);
	prototype_context_formation_certificate_db_init(
		&context_certificate_db, context_certificates, 128
	);
	...
	uint32_t box_is_type = add_claim(
		&judgement, PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
	...
	if (prototype_hott_action_certificate_add(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, context_action_certificate,
	...
	memcpy(
		context_result.calculus_fingerprint,
		PROTOTYPE_HOTT_CALCULUS_FINGERPRINT,
		65
	);
	uint32_t result_id;
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, type_action, &repeated_type_action_id
	...
	if (prototype_hott_action_certificate_add(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, wrong_kind_certificate,
			&(uint32_t){0}
		) == 0) {
		return 33;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, bool_type_action, &bool_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, bool_type_action_id, &result_id
		) != 0 || action_results[result_id].state !=
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, bool_term_action, &bool_term_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, bool_term_action_id,
			&bool_term_result_id
		) != 0 || action_results[bool_term_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
		prototype_hott_action_db_validate(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
	...
	if (bool_binding == PROTOTYPE_INVALID_ID || prototype_context_extend(
			...
			PROTOTYPE_INVALID_ID,
			&bool_context
		) != 0 || prototype_context_formation_certificate_db_add(
			&context_certificate_db,
			&context_db,
			&term_db,
			&type_db,
			&judgement,
	...
	if (prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, bool_type_action_id,
			&bool_type_result_id
	...
	if (prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, type_action_id, &result_id
		) != 0 || action_results[result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_UNSUPPORTED ||
		action_results[result_id].residual_reason !=
			PROTOTYPE_HOTT_RESIDUAL_HOST_PRIMITIVE) {
		return 47;
	}
	...
	if (prototype_hott_execute_substitution_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, identity_substitution_action_id,
			PROTOTYPE_TERM_NORMALIZATION_PURE_TYPE_WHNF, 32,
			&identity_action_result
		) != 0 || action_results[identity_action_result].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 48;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, projection_action,
			&projection_action_id
		) != 0 || prototype_hott_execute_substitution_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, projection_action_id,
			PROTOTYPE_TERM_NORMALIZATION_PURE_TYPE_WHNF, 32,
			&projection_result_id
		) != 0 || action_results[projection_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 50;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, composition_action,
			&composition_action_id
		) != 0 || prototype_hott_execute_substitution_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, composition_action_id,
			PROTOTYPE_TERM_NORMALIZATION_PURE_TYPE_WHNF, 32,
			&composition_result_id
		) != 0 || action_results[composition_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
		prototype_hott_action_db_validate(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, extension_action, &extension_action_id
		) != 0 || prototype_hott_execute_substitution_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, extension_action_id,
			PROTOTYPE_TERM_NORMALIZATION_PURE_TYPE_WHNF, 64,
			&extension_result_id
		) != 0 || action_results[extension_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
		prototype_hott_action_db_validate(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, contextual_bool_type_action,
			&contextual_bool_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, contextual_bool_type_action_id,
			&contextual_bool_type_result_id
		) != 0 || action_results[contextual_bool_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 58;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, bool_variable_action,
			&bool_variable_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, bool_variable_action_id,
			&bool_variable_result_id
		) != 0 || action_results[bool_variable_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 59;
	}
	...
	uint32_t boxed_claim = add_claim(
		&judgement,
	...
	boxed_derivation->constructor_owner_view = box_view;
	boxed_derivation->constructor_index = PROTOTYPE_INVALID_ID;
	boxed_derivation->constructor_field_index = PROTOTYPE_INVALID_ID;
	boxed_derivation->induction_match = PROTOTYPE_INVALID_ID;
	boxed_derivation->induction_motive = PROTOTYPE_INVALID_ID;
	boxed_derivation->induction_case_index = PROTOTYPE_INVALID_ID;
	boxed_derivation->induction_field_index = PROTOTYPE_INVALID_ID;
	...
	boxed_derivation->premise_claim_ids[0] = bool_variable_claim;
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, box_type_action, &box_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, box_type_action_id, &box_type_result_id
		) != 0 || action_results[box_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 76;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, boxed_term_action, &boxed_term_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, boxed_term_action_id,
			&boxed_term_result_id
		) != 0 || action_results[boxed_term_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 77;
	}
	...
	pure_box_computation_claim = add_claim(
		&judgement,
	...
	uint32_t returned_box_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_OPERATION,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, pure_box_type_action,
			&pure_box_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, pure_box_type_action_id,
			&pure_box_type_result_id
		) != 0 || action_results[pure_box_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 80;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, returned_box_action,
			&returned_box_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, returned_box_action_id,
			&returned_box_result_id
		) != 0 || action_results[returned_box_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 81;
	}
	...
	for (uint32_t i = 0; i < judgement.derivation_count; ++i) {
		if (judgement.derivations[i].conclusion_claim_id ==
			...
			judgement.derivations[i].premise_count == 4 &&
			judgement.derivations[i].premise_claim_ids[3] == boxed_witness_claim) {
	...
	uint32_t pure_box_thunk_type_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		pure_box_thunk_type,
	...
	uint32_t thunked_box_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_OPERATION,
	...
	judgement.derivations[judgement.derivation_count++] =
		(struct prototype_judgement_derivation){
			...
			.premise_count = 1,
			.premise_claim_ids = { returned_box_claim }
		};
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, pure_box_thunk_type_action,
			&pure_box_thunk_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, pure_box_thunk_type_action_id,
			&pure_box_thunk_type_result_id
		) != 0 || action_results[pure_box_thunk_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 84;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, thunked_box_action,
			&thunked_box_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, thunked_box_action_id,
			&thunked_box_result_id
		) != 0 || action_results[thunked_box_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 85;
	}
	...
	for (uint32_t i = 0; i < judgement.derivation_count; ++i) {
		if (judgement.derivations[i].conclusion_claim_id ==
			...
			judgement.derivations[i].premise_count == 4 &&
			judgement.derivations[i].premise_claim_ids[3] ==
	...
	uint32_t pi_type_in_bool_context_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		bool_to_pure_bool,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, pi_type_action, &pi_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, pi_type_action_id, &pi_type_result_id
		) != 0 || action_results[pi_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 88;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, app_result_type_action,
			&app_result_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, app_result_type_action_id,
			&app_result_type_result_id
		) != 0 || action_results[app_result_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 94;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, app_action, &app_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, app_action_id, &app_result_id
		) != 0 || action_results[app_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 95;
	}
	...
	uint32_t lambda_type_in_bool_context_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		bool_to_pure_bool_computation,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, lambda_type_action,
			&lambda_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, lambda_type_action_id,
			&lambda_type_result_id
		) != 0 || action_results[lambda_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 98;
	}
	...
	uint32_t pure_bool_type_in_bool_context_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		pure_bool_computation,
	...
	uint32_t returned_outer_bool_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_OPERATION,
	...
	judgement.derivations[judgement.derivation_count++] =
		(struct prototype_judgement_derivation){
			...
			.premise_count = 1,
			.premise_claim_ids = { outer_bool_in_lambda_body_claim }
		};
	...
	uint32_t lambda_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_OPERATION,
		lambda_operation,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, lambda_action, &lambda_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, lambda_action_id, &lambda_result_id
		) != 0 || action_results[lambda_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 103;
	}
	...
	uint32_t value_match_classifier_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
	...
	uint32_t value_match_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_OPERATION,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, value_match_type_action,
			&value_match_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, value_match_type_action_id,
			&value_match_type_result_id
		) != 0 || action_results[value_match_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 108;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, value_match_action,
			&value_match_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, value_match_action_id,
			&value_match_result_id
		) != 0 || action_results[value_match_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 109;
	}
	...
	uint32_t unbox_classifier_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		unbox_classifier,
	...
	uint32_t unbox_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_OPERATION,
		unbox_operation,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, unbox_type_action,
			&unbox_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, unbox_type_action_id,
			&unbox_type_result_id
		) != 0 || action_results[unbox_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 117;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, unbox_action, &unbox_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, unbox_action_id, &unbox_result_id
		) != 0 || action_results[unbox_result_id].state !=
	...
	uint32_t nat_is_type = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		nat_view,
	...
	uint32_t nat_classifier_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		nat_classifier,
	...
	uint32_t nat_ih_classifier_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_IS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_TYPE_FORMATION,
		nat_ih_classifier,
	...
	uint32_t nat_zero_in_context_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
	...
	uint32_t nat_ih_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_CORE_HELPER,
	...
	judgement.derivations[judgement.derivation_count++] =
		(struct prototype_judgement_derivation){
			...
			.induction_match = nat_match,
			.induction_motive = nat_motive,
			.induction_case_index = 1,
			.induction_field_index = 0,
			.premise_count = 0
		};
	...
	uint32_t nat_match_claim = add_claim(
		&judgement,
		PROTOTYPE_JUDGEMENT_KIND_HAS_TYPE,
		PROTOTYPE_JUDGEMENT_AUTHORITY_OPERATION,
		nat_match_operation,
	...
	judgement.derivations[judgement.derivation_count++] =
		(struct prototype_judgement_derivation){
			...
			.premise_count = 3,
			.premise_claim_ids = {
				nat_classifier_claim,
				nat_zero_in_context_claim,
				nat_ih_claim
			}
		};
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, nat_type_action, &nat_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, nat_type_action_id, &nat_type_result_id
		) != 0 || action_results[nat_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, nat_match_type_action,
			&nat_match_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, nat_match_type_action_id,
			&nat_match_type_result_id
		) != 0 || action_results[nat_match_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 131;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, nat_match_action, &nat_match_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, nat_match_action_id, &nat_match_result_id
		) != 0 || action_results[nat_match_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY || prototype_hott_action_db_validate(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, observation_type_action,
			&observation_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, observation_type_action_id,
			&observation_type_result_id
		) != 0 || action_results[observation_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
		prototype_hott_bridge_db_construct_extension(
			&bridge_db,
			&context_db,
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, higher_type_action,
			&higher_type_action_id
		) != 0 || prototype_hott_execute_type_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, higher_type_action_id,
			&higher_type_result_id
		) != 0 || action_results[higher_type_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY) {
		return 71;
	}
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, proof_variable_action,
			&proof_variable_action_id
		) != 0 || prototype_hott_execute_term_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, proof_variable_action_id,
			&proof_variable_result_id
		) != 0 || action_results[proof_variable_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, repeated_bool_extension_action,
			&repeated_bool_extension_action_id
		) != 0 || prototype_hott_execute_substitution_action(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, repeated_bool_extension_action_id,
			PROTOTYPE_TERM_NORMALIZATION_PURE_TYPE_WHNF, 64,
			&repeated_bool_extension_result_id
		) != 0 || action_results[repeated_bool_extension_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
		prototype_hott_action_db_validate(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
	...
	if (prototype_hott_observation_plan_and_execute(
		...
		action_results[distinct_execution.type_action_result_id].state !=
			PROTOTYPE_HOTT_ACTION_RESULT_READY ||
	...
	if (prototype_hott_action_request_intern(
			&action_db, &context_db, &substitution_db, &context_certificate_db,
			&substitution_certificate_db, &bridge_db, &term_db, &type_db,
			&operation_graph, &judgement, atomic_failure_request,
	...
	int atomic_failure_status = prototype_hott_execute_type_action(
		&action_db, &context_db, &substitution_db, &context_certificate_db,
		&substitution_certificate_db, &bridge_db, &term_db, &type_db,
		&operation_graph, &judgement, atomic_failure_request_id, &ignored_result_id
	);
