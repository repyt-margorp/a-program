...
src/artifact/         interface publication, v82 wire format, relocation, link
...
- `include/a_program/artifact/`, `src/artifact/`: artifact interface,
  publication closure, v82 wire reader/writer, relocation, and linking.
...
- Result evidence uses ordinary TermDB application spines plus accepted Claims
  ...
  fragment covers finite deterministic evaluation, structural/IH/Acc totality,
  ...
  recursion, nondeterministic may/must termination, and general function-graph
  postconditions are not implemented.
...
`A_PROGRAM_ARTIFACT 82 <calculus-fingerprint>`. The reader accepts that version
...
boundary are specified by `spec/artifact_v82.schema`; the implemented
...
v82 debug/name records are readable, that term exports keep distinct classifier
